...
class DepthPerceptionNode(Node):
    ...
    def depth_callback(self, msg: Image):
        ...
        out.passability = float(np.clip(0.5 + 0.8 * decision.geom_margin + 0.3 * decision.obs_confidence, 0.0, 1.0))
        out.risk = float(np.clip(1.0 - out.passability, 0.0, 1.0))
    ...
    def analyze_depth(self, depth: np.ndarray) -> DepthDecision:
        ...
        def near_min(arr):
            ...
            return float(np.min(arr[mask])) if np.any(mask) else float('inf')
